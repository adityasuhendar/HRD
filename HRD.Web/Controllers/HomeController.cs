    ...
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> HRDDashboard()
        {
            ...
            try
            {
                ...
                var karyawanResult = await _apiService.GetAsync<List<dynamic>>("api/karyawan");
                if (karyawanResult?.Success == true && karyawanResult.Data != null)
                {
                    model.TotalKaryawan = karyawanResult.Data.Count;
                }

                // Get today's attendance summary  
                var todaySummaryResult = await _apiService.GetAsync<object>("api/presensi/today-summary");
                if (todaySummaryResult?.Success == true && todaySummaryResult.Data != null)
                {
                    try
                    {
                        var jsonElement = (System.Text.Json.JsonElement)todaySummaryResult.Data;
                        
                        // Extract attendance data from nested data property
                        if (jsonElement.TryGetProperty("data", out var dataElement))
                        {
                            if (dataElement.TryGetProperty("sudahClockIn", out var sudahClockIn))
                            {
                                model.KaryawanHadirHariIni = sudahClockIn.GetInt32();
                            }
                            
                            if (dataElement.TryGetProperty("totalKaryawan", out var totalKaryawan))
                            {
                                model.TotalKaryawan = totalKaryawan.GetInt32();
                            }
                        }
                        
                        // Calculate attendance rate
                        if (model.TotalKaryawan > 0)
                        {
                            model.AbsensiRate = (decimal)model.KaryawanHadirHariIni / model.TotalKaryawan * 100;
                        }
                        else
                        {
                            model.AbsensiRate = 0;
                    ...
                    {
                        // If JSON parsing fails, try to get basic employee count
                        ViewBag.Warning = $"Could not parse attendance data: {jsonEx.Message}";
                    }
                }
            }
            catch (Exception ex)
